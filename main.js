...
if (window.innerWidth <= 700) {
  ...
  if (mobileResume) {
    ...
    import(
      ...
      const pdf = await pdfjsLib.getDocument("/Portfolio/resume.pdf").promise;

      for (let pageNumber = 1; pageNumber <= pdf.numPages; pageNumber++) {

        const page = await pdf.getPage(pageNumber);

        const containerWidth = mobileResume.clientWidth - 20;
        ...
        mobileResume.appendChild(canvas);
      ...
      console.error("Mobile resume viewer error:", error);
    });
  }
}